...
const Router =  {
  // data() { return {
    ...
    // hooks that are called for each route, functions for 'before' and 'after'.
    hookFunctions: {},
  // }},
  // methods: {
    // Note: Call right after creating all of your routes.
    init: function(zeroFrame) {
      this.page = zeroFrame
      ...
      if (address == '') {
        this.page.cmd("wrapperPushState", [{
          "route": ""
        }, "", this.root]);
      }
        ...
        if (match) {
          ...
          if (this.hookFunctions && this.hookFunctions["before"]) { // TODO: Move this into navigate function?
            if (!this.hookFunctions["before"].call(object, this.routes[i].path, routeParams, this.searchQuery)) {
              ...
              }, null, this.root + this.clearSlashes(this.currentRoute)]);
              return this;
            }
          }
          ...
          // Vue.set(this, 'currentRoute', this.routes[i].path)
    ...
    navigate: function(path, doPush = true) {
      ...
      if (doPush) {
        this.page.cmd("wrapperPushState", [{
          "route": path
        }, path, this.root + this.clearSlashes(path)]);
      }
      this.check(path);
      return this;
    },
